...
class TwitterUpdates:
  ...
  def update_retweet(self, retweeted_id, data, party): 
    ...
    self.update_user(data['user']['id'], 'retweet', data['user'], party)
    self.write_record(ret, party + '_retweets')
  ...
  def update_user(self, user_id, action, user_info, party):
    ret = {}
    ...
    ret['friends'] = None
    ret['followed_cnts'] = 0
    self.write_record(ret, party + '_users')

  def update_tweet(self, data, party):
    ret = {}
    ...
    ret['user'] = data['user']['id']
    ret['processed'] = False
    self.update_user(ret['user'], 'tweet', data['user'], party)
    ...
    self.write_record(ret, party + "_tweets")
    self.cnts += 1
